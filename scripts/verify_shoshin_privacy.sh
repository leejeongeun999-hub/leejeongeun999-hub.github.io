...
const relativePath = 'privacy.html';
...
for (const phrase of forbiddenParts) {
  if (html.toLowerCase().includes(phrase.toLowerCase())) {
    throw new Error(`stale Shoshin advertising statement remains: ${phrase}`);
  }
}
for (const phrase of [
  ...
  if (!html.includes(phrase)) throw new Error(`required Shoshin statement missing: ${phrase}`);
}
...
server.listen(0, '127.0.0.1', async () => {
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${port}/privacy.html`);
    const body = await response.text();
    if (!response.ok || body !== html) throw new Error('served Shoshin privacy page differs from verified source');
